...
transformations: a non-linear relationship, help with outliers (doing a logarithmic transformation takes large values and makes them smaller)

```{r model 1}
model1 <- lm(homeless ~ funding_total, data = states_clean)
summary(model1) 
plot(homeless ~ funding_total, data = states_clean)
abline(model1)
```
...
plot(homeless ~ funding_tot, data = county_clean)
```
